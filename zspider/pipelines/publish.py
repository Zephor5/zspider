...
class PubPipeLine(object):
    ...
    def process_item(self, item, spider):
        ...
        if self._pub:
            ...
            d.addCallback(lambda doc: logger.debug(json.dumps(doc, indent=4), extra=extra) or doc)
            d.addCallback(lambda doc: http.twisted_post(self.PUB_PORT, urllib.urlencode({'doc': json.dumps(doc)})))
    ...
    def process_trans(self, _doc, item, extra):
        for x in self.tans:
            ...
            try:
                ...
                logger.warn(u'get error :{0:s}, when eval :{1:s}'.format(e.message, x), extra=extra)
                _doc = __doc
            else:
                if 'trash' in _doc:
                    logger.info(u'skip item :{0:s} with filter :{1:s}'.format(_doc['title'], x), extra=extra)
                    item['status'] = zsm.STATUS_PUB_SKIP
                    item['info'] = u'发布过滤：%s' % x
                    raise _SkipDoc
        return _doc

    def prepare_data(self, _doc, item):
    ...
    def after_pub(res, item, extra):
        ...
        if isinstance(data, dict):
            ...
            item['info'] = u'发布成功:%s' % data
            logger.info('发布成功: %s' % res, extra=extra)
        else:
            item['status'] = zsm.STATUS_PUB_FAIL
            item['info'] = unicode(data)
            logger.warn('发布失败：%s' % res, extra=extra)
        return item
    ...
    def post_fail_or_skip(failure, item, extra):
        if failure.type is _SkipDoc:
            return item
        item['status'] = zsm.STATUS_PUB_FAIL
        item['info'] = unicode(failure)
        logger.error(u'调用发布接口失败：{0:s}'.format(failure), extra=extra)
        return item
