...
class TaskConfNewspaper(fm.BaseTaskConf):
    page_title_xpath = fm.XPathField(
        required=True, verbose_name="版面标题xpath", help_text="版面块抬头xpath，仅包含版面号及名称"
    )
    page_name_no_re = fm.RegExpField(
        required=True,
        group_num=2,
...
class Newspaper(BaseNewsParser):
    ...
    def __init__(self, task_id, task_name, task_conf=None, article_fields=None):
        super(self.__class__, self).__init__(
            task_id, task_name, task_conf, article_fields
        )
    ...
    def _parse_page(self, page):
        ...
        for new in page.xpath(self._conf.page_news_xpath):
            new = {
                "title": "".join(new.xpath(self.INNER_TEXT).extract()).strip(),
        ...
        page = {"name": page_name, "news": page_news, "status": UNREAD, "read": 0}
