...
从现有 Spider、Parser 和数据模型看，ZSpider 主要瞄准的不是“任意网页”，而是比较稳定的内容流抓取场景，核心路径是“索引页发现内容 -> 详情页抽取字段”。

当前最适合的目标形态主要有三类：

- **标准资讯/新闻/公告站点**：有列表页或频道页，列表中能提取详情链接，详情页中能提取标题、正文、发布时间、来源
- **公众号文章流**：以公众号或文章列表为入口，重点抓文章正文和元数据
- **依赖 JavaScript 渲染的内容页面**：页面本身仍然是内容详情页形态，只是索引页或详情页需要渲染后再抽取
...
- **强交互业务流页面**：例如下单、支付、复杂多步表单
- **接口编排型采集**：以 App / OpenAPI 鉴权、签名、接口拼装为核心
- **高度个性化站点**：需要复杂状态保持、内容实时变化很快
- **通用全站遍历**：以广度优先发现和整站抓取为目标的通用爬虫
- **非文章型主模型**：例如商品、订单、评论线程、社交关系、地图轨迹

这和它的数据模型也是一致的。系统默认沉淀的是标题、正文、发布时间、来源、URL 这类文章型字段，而不是复杂业务对象。
